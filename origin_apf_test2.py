...
def get_norm_vec(v1,v2):
    ...
    if not (f1 or f2):
        ret1 = np.array([1, -v1[0] / v1[1]])
        ret2 = np.array([-1, v1[0] / v1[1]])
        if cal_angle(v2,ret1)>=90:
            ret = ret1
        elif cal_angle(v2,ret2)>=90:
            ret = ret2
...
class Traditional_Apf_Method:
    ...
    def get_next_point(self, pos_now, pos_prev, external_obs_list, external_eta_list, external_input=False):
        ...
        if not external_input:  # traditonal
            ...
            force = np.zeros(3)
            for idx, obs in enumerate(obs_list):
                force += self.repulsion_one_only(pos_now, 0.6, obs)
            ...
            if not (np.linalg.norm(force[0:2]) < 1e-7 or np.linalg.norm(attract_force[0:2]) < 1e-7):
                if abs(cal_angle(-force[0:2], attract_force[0:2])) <= 5:
                    norm_force = get_norm_vec(attract_force[0:2],-force[:2]) * np.linalg.norm(attract_force) * 0.2
        ...
        if not (np.linalg.norm(rep_force[0:2]) < 1e-7 or np.linalg.norm(attraction_force[0:2]) < 1e-7):
            if abs(cal_angle(-rep_force[0:2], attraction_force[0:2])) <= 5:
                norm_force = get_norm_vec(attraction_force[0:2],-rep_force[:2]) * np.linalg.norm(attraction_force) * 1
...
class apf_env:
    def __init__(self, pcd_path, start, end):
        ...
        # self.pcd=self.pcd.voxel_down_sample(voxel_size=0.5)
        self.pcd_tree = o3d.geometry.KDTreeFlann(self.pcd)
        self.global_vox_map = o3d.geometry.VoxelGrid.create_from_point_cloud(
            self.pcd,0.5
        )
    ...
    def get_observation(self, pos_now):
        ...
        if not self.vox_map:
            self.center_coordinates = None
            center_coordinates = np.zeros((200, 3))
        else:
            ...
            if 0 < num < 200:
                center_coordinates = np.vstack((center_coordinates, np.zeros((200 - num, 3))))
            elif num >= 200:
                center_coordinates = center_coordinates[:200]
            else:
                center_coordinates = np.zeros((200, 3))

        self.observation_now = (np.array(center_coordinates), np.concatenate((pos_now, self.end)))
    ...
    def get_next_point(self, pos_now, rep_coeff, attract_coeff, step_length):
        ...
        if not self.vox_map:
            ...
            # attraction_force = self.attraction(pos_now, attract_coeff)
            attraction_force = -self.repulsion_one_only(pos_now,attract_coeff,self.end)
            rep_force = np.zeros(3)
        ...
        if not (np.linalg.norm(rep_force[0:2]) < 1e-7 or np.linalg.norm(attraction_force[0:2]) < 1e-7):
            if abs(cal_angle(-rep_force[0:2], attraction_force[0:2])) <= 35:
                print("CASE")

                norm_force = get_norm_vec(attraction_force[:2],-rep_force[:2]) * 10*max(np.linalg.norm(rep_force),
                                                                                     np.linalg.norm(attraction_force))
                rep_force[:2] += norm_force
                # rep_force[:2] = norm_force
    ...
    def kinamatic_constrain(self, pos_next):
        ...
        if angle <= max_yaw:
            ...
            if theta1 >= 0 and theta2 >= 0:
                ...
                if dtheta >= 0:
                    ret = np.array(
                        [d * np.cos(theta1 + max_yaw / 180 * np.pi), d * np.sin(theta1 + max_yaw / 180 * np.pi)])
                else:
                    ret = np.array(
                        [d * np.cos(theta1 - max_yaw / 180 * np.pi), d * np.sin(theta1 - max_yaw / 180 * np.pi)])
    ...
    @staticmethod
    def map_interval(origin,new_low,new_high):
        k1 = (new_high+new_low)/(new_high-new_low)
        k2 = 2/(new_high-new_low)
        return (origin+k1)/k2
    def step(self, action):
        # CASE
        # rep = (action[:-2] + 2.636363) / 1.81818  # 0.9 2.
        # attract = (action[-2] + 2) / 2  # 0.5,1.5
        # step_length = (action[-1] + 2) / 10  # 0.1 0.15 #0.1 0.2 #0.1 0.3

        rep = self.map_interval(action[:2],0.6,0.61)
        attract = self.map_interval(action[-2],2.1,2.11)
        step_length = self.map_interval(action[-1],0.2,0.21)
        #print(rep)
...
if __name__ == "__main__":
    ...
    root_path = r"./pcd/rollout_21-02-07_03-14-08_"
    ...
    apf = Traditional_Apf_Method(env_path=
                                 r"./pcd/rollout_21-02-07_03-14-08_pointcloud-unity.ply"
                                 , start=start, end=end, step_size_dim=step_size_eta_dim,
